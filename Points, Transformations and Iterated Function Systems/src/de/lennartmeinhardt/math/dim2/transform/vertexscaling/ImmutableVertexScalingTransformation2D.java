...
public final class ImmutableVertexScalingTransformation2D implements VertexScalingTransformation2D {
	
	private final Point2D vertex;
	private final double factor;

	public ImmutableVertexScalingTransformation2D(double vertX, double vertY, double factor) {
		this.vertex = new ImmutablePoint2D(vertX, vertY);
		this.factor = factor;
	}
	
	public ImmutableVertexScalingTransformation2D(Point2D point, double factor) {
